...
namespace MobileShopOnline.Controllers
{
    public class CartController : Controller
    {
        ...
        public ActionResult GetCartInfo()
        {
            ...
            // Giá tổng mặc định không có mã giảm giá

            if (TempData["DiscountAmount"] != null)
            {
                discountAmount = (decimal)TempData["DiscountAmount"];
            }
            ViewBag.TotalPrice = totalPrice - discountAmount;
            Session["TotalPrice"] = ViewBag.TotalPrice;
            ViewBag.DiscountAmount = discountAmount;
            Session["Dagiam"] = ViewBag.DiscountAmount;
        ...
        {
            List<CartItem> myCart = GetCart();
            // Tính toán tổng giá trị FinalPrice từ danh sách myCart
            ...
            ViewBag.DiscountAmount = Session["Dagiam"];
            ViewBag.FinalPrice = totalFinalPrice;
            return View(myCart);
        }
        ...
        [HttpPost]
        public ActionResult Update(string CouponName)
        {
            var coupon = db.Coupons.FirstOrDefault(c => c.CouponName == CouponName);
            decimal discountAmount = 0;
            if (coupon != null)
            {
                decimal couponPercentage = coupon.CouponPercentage ?? 0;
                decimal totalPrice = GetTotalPrice();
                discountAmount = totalPrice * (couponPercentage / 100);
            }
            TempData["DiscountAmount"] = discountAmount; // Lưu giá trị vào TempData để sử dụng trong request tiếp theo
            return RedirectToAction("GetCartInfo", "Cart");
        }
